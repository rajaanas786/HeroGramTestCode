...
const MediaView = () => {
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        const response = await axios.get(`http://http://159.65.122.248:5000/api/videos/${id}`);
...
<video controls className="w-full">
  <source src={`http://http://159.65.122.248:5000/uploads/videos/${media.filename}`} type="video/mp4" />
  Your browser does not support the video tag.
</video>
          ...
          <div>
            <img src={`http://http://159.65.122.248:5000/uploads/images/${media.filename}`} alt="Media" className="w-full" />
          </div>
        )}
        <div className="mt-4">
          <button
